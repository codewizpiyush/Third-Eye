...
function WriteExam() {
  ...
  const cheatSubmit = async () => {
    if (isSubmitted || view === "result" || view === "review") return;
    setIsSubmitted(true);
    ...
    let verdict = correctAnswers.length < examData.passingMarks ? "Fail" : "Pass";
  ...
  useEffect(() => {
    if (view === "questions") {
      const examRoot = document.getElementById("root");
      if (examRoot?.requestFullscreen) examRoot.requestFullscreen();

      const handleFullscreenChange = () => {
        if (!document.fullscreenElement) {
          alert("Exited full screen. Submitting exam.");
          cheatSubmit();
        }
      };
  ...
  useEffect(() => {
    if (timeUp && view === "questions") cheatSubmit();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [timeUp]);
