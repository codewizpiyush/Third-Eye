...
router.put("/verify-exam/:id", authMiddleware, async (req, res) => {
  try {
    const exam = await Exam.findById(req.params.id);
    if (!exam) {
      return res.status(404).send({ success: false, message: "Exam not found" });
    }
    exam.isVerified = req.body.isVerified; // <-- take from frontend
    await exam.save();
    res.send({
      success: true,
      message: `Exam ${exam.isVerified ? "verified" : "unverified"} successfully`,
      data: exam,
    });
  } catch (error) {
